...
window.onload = function() {
    ...
    var queue = {
        ...
        paint: function() {
            var str = "";
            if(str.length<=60){
                each(this.str, function(item){str += ("<div title=\""+parseInt(item)+"\" style=\"height:"
                    +parseInt(item)+"px\"></div>")});
                container.innerHTML = str;
                addDivDelEvent();
            }
            else{
                alert("the queue is limited to 60 elements!");
            }
...
function BubbleSort() {
        ...
        Clock = setInterval(function() {
            ...
            if (parseInt(queue.str[i]) > parseInt(queue.str[i + 1])) {
    ...
    addEvent(buttonList[1], "click", function() {
        ...
        if ((/^[0-9]+$/).test(input)) {
            if(10<=parseInt(input)&&parseInt(input)<=100){
                queue.leftPush(input);
            }
            else{
                alert("Please enter interger between 10-100!");
            }
        }
        else {
    ...
    addEvent(buttonList[2], "click", function() {
        var input = buttonList[0].value;
        if ((/^[0-9]+$/).test(input)) {
            if(10<=Number(input)&&Number(input)<=100){
